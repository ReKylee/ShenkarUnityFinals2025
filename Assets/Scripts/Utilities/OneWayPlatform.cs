    ...
    public class OneWayPlatform2D : MonoBehaviour
    {
        ...
        private void CheckCollisions()
        {
            ...
            for (int i = 0; i < contactCount; i++)
            {
                ...
                Vector2 platformTop = new Vector2(transform.position.x, 
        ...
        private void CheckNearbyObjects()
        {
            ...
            Collider2D[] nearbyColliders = Physics2D.OverlapBoxAll(checkCenter, checkSize, 0f, platformLayers);
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            if (_platformCollider != null)
            {
                ...
                Vector3 size = new Vector3(_platformCollider.size.x, _platformCollider.size.y, 0.1f);
                Vector3 center = new Vector3(_platformCollider.offset.x, _platformCollider.offset.y, 0f);
                
                Gizmos.DrawWireCube(center, size);
                
                // Draw the platform top detection line
                Gizmos.color = Color.cyan;
                Vector3 topLineStart = center + Vector3.left * _platformCollider.size.x / 2f + Vector3.up * (_platformCollider.size.y / 2f + detectionHeight);
                Vector3 topLineEnd = center + Vector3.right * _platformCollider.size.x / 2f + Vector3.up * (_platformCollider.size.y / 2f + detectionHeight);
                Gizmos.DrawLine(topLineStart, topLineEnd);
                
                // Draw the solid platform surface
                Gizmos.color = Color.red;
                topLineStart = center + Vector3.left * _platformCollider.size.x / 2f + Vector3.up * _platformCollider.size.y / 2f;
                topLineEnd = center + Vector3.right * _platformCollider.size.x / 2f + Vector3.up * _platformCollider.size.y / 2f;
                Gizmos.DrawLine(topLineStart, topLineEnd);
            }
        }
    }
