...
namespace LevelSelection
{
    ...
    public class LevelSelectionDirector
    {
        public List<LevelData> BuildLevelData(List<GameObject> levelGameObjects)
        {
            ...
            {
                GameObject levelObject = levelGameObjects[i];
                if (levelObject == null) continue;

                // Try to create from LevelPoint component first
                LevelData levelData = LevelDataFactory.CreateFromGameObject(levelObject, i);

                if (levelData == null)
                {
                    // Fallback - create from transform position and object name
                    Debug.Log(
                        $"Creating level data from transform for {levelObject.name} at position {levelObject.transform.position}");
